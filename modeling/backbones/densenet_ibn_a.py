...
class DenseNet(nn.Module):
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'norm.1' in name or 'relu.1' in name  or 'conv.1' in name:
                t=name.split('.')
                index=t.index('1')
                name1='.'.join(t[:index])
                name2='_'.join([name1,t[index]])
                name='.'.join([name2]+t[index+1:])
            if 'norm.2' in name or 'relu.2' in name  or 'conv.2' in name:
                t=name.split('.')
                index=t.index('2')
                name1='.'.join(t[:index])
                name2='_'.join([name1,t[index]])
