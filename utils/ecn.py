...
def ECN(queryset,testset,k=25,t=3,q=8,method='rankdist',MemorySave = True,Minibatch = 2000):
    ...
    nQuery = queryset.size(0)
    ntest = testset.size(0)
    all_num = nQuery + ntest

    feat = torch.cat([queryset,testset])
    if MemorySave or ntest>50000:
        print('MemorySave Mode')
        r_dist = np.zeros(shape=[all_num,all_num],dtype=np.float16)
        i = 0
        while True:
            it = i + Minibatch
            if it < np.shape(feat)[0]:
                r_dist[i:it,:] = torch.mm(feat[i:it,:],feat.t()).cpu().numpy()
            else:
                r_dist[i:,:] = torch.mm(feat[i:,:],feat.t()).cpu().numpy()
                break
            i = it
    else:
        r_dist = torch.mm(feat,feat.t()).cpu().numpy()
    r_dist = 1 - r_dist
    del feat
